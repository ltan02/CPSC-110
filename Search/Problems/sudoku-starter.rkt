...
;; produce the solved version of the board, otherwise false
...
(@template Board (listof Board) genrec backtracking arb-tree encapsulated)
(define (solve bd)
...
;; true if the board is complete with no blank spaces, otherwise false
...
;; produce the next boards that are valid
;; ASSUME: board is not solved
...
(define (next-boards b0)
  (local [(define (get-next-empty b)
            (if (false? (first b))
                0
                (+ 1 (get-next-empty (rest b)))))

          (define (insert-numbers board empty-pos)
            (local [(define (build-board n)
                      (fill-square board empty-pos (+ n 1)))]
              (build-list 9 build-board)))
          ...
          (define (valid? b1)
            (local [(define (valid-units? lou)
                    ...
                    (define (read-unit u)
                      (local [(define (read-pos p)
                                (read-square b1 p))]
                        (map read-pos u)))

                    (define (keep-numbers-only lov)
                      (local [(define (not-false? v)
                                (not (false? v)))]
                        (filter not-false? lov)))
  ...
  (filter valid? (insert-numbers b0 (get-next-empty b0)))))
