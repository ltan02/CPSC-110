...
;; sequence.
;;
;; https://s3.amazonaws.com/edx-course-spdx-kiczales/HTC/problems/tsolitaire.png
...
(@htdd Position)
;; Position is Natural
;; interp. 0 based indexes into a board in range [0, 14]
...
(@template genrec arb-tree backtracking encapsulated)

(define (solve bd)
  ;; Termination argument:
  ;;  each step removes 1 peg from the board
  ;;  stops when it is not possible to remove anymore pegs
  ;;  (solved or no valid jumps)
...
(@template fn-composition use-abstract-fn encapsulated Jump)
  ...
  (local [;; Jump -> Board
          (define (do-jump j)
            (local [(define from (jump-from j))
              ...
              (set-cell (set-cell (set-cell bd to true) over false)
                        from
                        false)))
