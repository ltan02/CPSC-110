...
;; true if the room with name s is found, otherwise false
(check-expect (lookup-room H1 "B") true)
(check-expect (lookup-room H1 "C") false)
(check-expect (lookup-room H4 "E") true)
(check-expect (lookup-room H4 "G") false)

;(define (lookup-room r s) false) ;stub

(@template Room (listof Room) encapsulated accumulator)
(define (lookup-room r0 s)
  ;; todo is (listof Room) ; worklist accumulator
  ;; visited is (listof Room) ; rooms that were already visited
  (local [(define (fn-for-room r todo visited)
            (cond [(string=? (room-name r) s) true]
                  [(member r visited) (fn-for-lor todo visited)]
                  [else
                   (fn-for-lor (append (room-exits r) todo)
                               (cons r visited))]))
