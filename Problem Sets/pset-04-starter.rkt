...
;; For problem set 4 you must improve this program in important way. Be sure
...
(define LOB1 empty)
(define LOB2 (cons (make-ball 100 200 3 4) empty))
...
;; start the game, call with (main empty)
...
;; removes all the balls in the list when " " is pressed
(check-expect (handle-key (cons (make-ball 1 2 3 4) empty) " ") empty)
(check-expect (handle-key empty " ") empty)
(check-expect (handle-key (cons (make-ball 1 2 3 4) empty) "a")
              (cons (make-ball 1 2 3 4) empty))

#;
(define (handle-key lob ke) lob)
...
;; render lob on image at appropriate x, y coordinates
(check-expect (render-balls empty) MTS)
(check-expect (render-balls (cons (make-ball 1 200 -2 3) empty))
              (place-image BALL 1 200 MTS))
(check-expect (render-balls (cons (make-ball 200 3 -2 -4)
                                  (cons (make-ball 230 219 2 -3) empty)))
              (place-image BALL 200 3
                           (place-image BALL 230 219 MTS)))

#;
(define (render-balls lob) MTS)

(@template ListOfBall)
(define (render-balls lob)
  (cond [(empty? lob) MTS]
        [else
         (render-ball (first lob) (render-balls (rest lob)))]))
...
(check-expect (render-ball (make-ball (- WIDTH 4) (- HEIGHT 5) -2 -3) MTS)
...
(@htdf next-balls)
(@signature ListOfBall -> ListOfBall)
;; produces a list of ball all at their next x,y
(check-expect (next-balls empty) empty)
(check-expect (next-balls (cons (make-ball (+ LEF 2) 200 -3 3) empty))
              (cons (make-ball (+ TOP 1) 200 3 3) empty))
(check-expect (next-balls (cons (make-ball 200 (+ TOP 3) -2 -4)
                                (cons (make-ball 230 219 2 -3) empty)))
              (cons (make-ball 200 (+ TOP 1) -2 4)
                    (cons (make-ball 232 216 2 -3) empty)))
...
(check-random (handle-mouse empty 100 200 "button-down")
              (cons (make-ball 100 200 (- 5 (random 11)) (- 5 (random 11)))
                    empty))
#;
(define (handle-mouse lob x y me) b)
