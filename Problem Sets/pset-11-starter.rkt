...
;; produces the total amount of gold from opening all the reachable treasure
(check-expect (reachable-gold T7) 100)
(check-expect (reachable-gold T1) 287)

;(define (reachable-gold t) 0) ;stub

(@template Treasure Route (listof Route) encapsulated accumulator)
(define (reachable-gold t0)
  ;; todo is (listof Route) ; worklist accumulator
  ;; visited is (listof Treasure) ; treasures seen already
  ;; rsf is Natural ; total amount of gold from teasures seen so far
  (local [(define (fn-for-treasure t todo visited rsf)
    ...
    (fn-for-treasure t0 empty empty 0)))
...
(@template Treasure Route (listof Route) encapsulated accumulator)
(define (all-labels t0)
  ;; todo is (listof Route) ; worklist accumulator
  ;; visited is (listof Treasure) ; treasures seen already
  ;; rsf is (listof String) ; names of treasures seen so far
  (local [(define (fn-for-treasure t todo visited rsf)
            (if (member t visited)
                (fn-for-lor todo visited rsf)
                (fn-for-lor (append (treasure-routes t) todo)
                            (cons t visited)
                            (append rsf (list (treasure-label t))))))
    ...
    (fn-for-treasure t0 empty empty empty)))
...
(define (short-dur-reachable t0 n)
  ;; todo is (listof Route) ; worklist accumulator
  ;; visited is (listof Treasure) ; treasures seen already
  ;; rsf is (listof String) ; treasures following routes < n hours long so far
  (local [(define (fn-for-treasure t todo visited rsf)
            (if (member t visited)
                (fn-for-lor todo visited rsf)
                (fn-for-lor (append (treasure-routes t) todo)
...
;;   - (equal? T1 T2) produces false
;;   - (equal? T3 T3) produces true
;;
;; Your solution MUST be tail recursive.
;;
...
;(define (num-lead-to t1 t2) 0)     ;stub

(@template Treasure Route (listof Route) encapsulated accumulator)
(define (num-lead-to t1 t2)
  ;; todo is (listof Route) ; worklist accumulator
  ;; visited is (listof Treasure) ; treasures seen already
  ;; rsf is Natural ; number of routes leading to t2 seen so far
  (local [(define (fn-for-treasure t todo visited rsf)
            (if (member t visited)
                (fn-for-lor todo visited rsf)
                (fn-for-lor (append (treasure-routes t) todo)
          ...
          (define (fn-for-route r todo visited rsf)
            (if (equal? (route-destination r) t2)
                (fn-for-treasure (route-destination r)
                                 todo
                                 visited
                                 (add1 rsf))
...
(@template Treasure Route (listof Route) encapsulated accumulator backtracking)
(define (route-to t s)
  ;; visited is (listof Treasure) ; treasures seen already
  ;; rsf is Natural ; duration of route so far
  (local [(define (fn-for-treasure t visited rsf)
            (cond [(member t visited) false]
...
(define (min-route-to t s)
  ...
  (local [(define (fn-for-treasure t visited rsf)
          ...
          (define (fn-for-lor lor visited rsf)
            (cond [(empty? lor) false]
                  [else
                   (local [(define try1 (fn-for-route (first lor) visited rsf))
