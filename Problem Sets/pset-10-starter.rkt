...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname pset-10-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (max-num-repeats los0)
  ;; prev is String ; the previous string in the list
  ;; max-string is Natural ; the max consective string seen so far
  ;; counter is Natural ; the current number of consective strings seen so far
  (local [(define (solve los prev max-string counter)
            (cond [(empty? los) (max max-string counter)]
                  [else
                   (if (string=? prev (first los))
                       ...
                       (solve (rest los)
                              (first los)
                              (max max-string counter)
                              1))]))]
    (if (empty? los0)
        0
        (solve (rest los0) (first los0) 0 1))))
...
(define (list-range loi0)
  ;; max-i is Natural ; the largest number seen so far
  ;; min-i is Natural ; the smallest number seen so far
  (local [(define (solve loi max-i min-i)
            (cond [(empty? loi) (- max-i min-i)]
                  [else
                   (solve (rest loi)
                          (max (first loi) max-i)
                          (min (first loi) min-i))]))]
    (if (= (length loi0) 1)
        0
        (solve (rest loi0) (first loi0) (first loi0)))))
...
;(define (in-alphabetical-order? los) false) ;stub

(@template (listof String) accumulator)
(define (in-alphabetical-order? los0)
  ;; prev is String ; the previous stirng in the list
  (local [(define (solve los prev)
            (cond [(empty? los) true]
                  [else
                   (if (string-ci<=? prev (first los))
                       (solve (rest los) (first los))
                       false)]))]
    (if (<= (length los0) 1)
        true
        (solve (rest los0) (first los0)))))
...
(define (filter-credits-course-num c0 course-num num-credits)
  ;; todo is (listof Course) ;; worklist accumulator
  ;; rsf is (listof Course) ;; courses that fit the criteria seen so far
...
;;
;; HINTS:
;;   - this is a compound worklist entry problem, if you aren't sure about
