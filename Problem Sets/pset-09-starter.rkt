...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname pset-09-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (solve (list (make-slot "A" 1) (make-slot "B" 1))
                     (list (make-ta "Ali" (list "B"))
                           (make-ta "Azi" (list "A"))))
              (list (list "Ali" "B") (list "Azi" "A")))
(check-expect (solve SLOTS1 TAS1)
              (list 
