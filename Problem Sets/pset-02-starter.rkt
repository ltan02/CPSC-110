...
;; Produces true if the rating is 1/2/4/5
...
(define (fn-for-altitude a)
  (cond [(and (string? a) (string=? "pre-launch")) (...)]
        [(number? a) (... a)]
        [else (...)]))
...
(check-expect (inflight? "post-launch") false)

;(define (inflight? a) false) ;stub

(@template Altitude)
(define (inflight? a)
  (cond [(and (string? a) (string=? "pre-launch")) false]
        [(number? a) true]
        [else false]))