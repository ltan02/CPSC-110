...
;;            - listed with the number of days left between 0 and 30, in days
;;            - reserved by a buyer
;;            - sold to a buyer
(define S1 0)
(define S2 15)
  ...
  (cond [(empty? los) empty]
        [else
         (if (> (string-length (first los)) n)
             (cons (first los) (strings-of-len (rest los) n))
...
(define (strings-of-len los n)
  (filter (λ (s) (> (string-length s) n)) los))
...
#;#;
(@template (listof String) encapsulated accumulator)
(define (strings-of-len los0 n)
  ;; rsf is (listof String) ; Strings that have length > n seen so far
...
(@signature (listof Image) -> Number)
;; produces the length of the longest sequence of images that increase in area
(check-expect (longest-sequence LOI0) 0)
(check-expect (longest-sequence LOI1) 3)
(check-expect (longest-sequence LOI2) 1)
(check-expect (longest-sequence LOI3) 1)
(check-expect (longest-sequence LOI4) 2)

;(define (longest-sequence loi) 0) ;stub

(define (longest-sequence loi0)
  ;; prev is Image ; the previous image
  ;; cur is Natural ; the current sequence of images that increase in area
  ;; max-seq is Natural ; the max sequence of images that increase in area
  (local [(define (fn-for-loi loi prev cur max-seq)
            (cond [(empty? loi) (max cur max-seq)]
                  [else
          ...
          (define (area>? img1 img2)
            (> (* (image-width img1) (image-height img1))
               (* (image-width img2) (image-height img2))))]
    (if (empty? loi0)
        0
        (fn-for-loi (rest loi0) (first loi0) 1 0))))
...
(@template Word Mutation (listof Mutation) encapsulated accumulator)
(define (more-add-than-remove? w0 x)
  ;; todo is (listof Mutation) ; worklist accumulator
  ;; visited is (listof Word) ; word visited already
  ;; rsf is Integer ; the difference between add and remove mutations
