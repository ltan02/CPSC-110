...
(@assignment accumulators-p11)
(@cwl ??? ???)

;; Consider the following house diagram:
;; 
;; https://s3.amazonaws.com/edx-course-spdx-kiczales/HTC/problems/house.png
;; 
...
;; using a tree:
;; 
;; https://s3.amazonaws.com/edx-course-spdx-kiczales/HTC/problems/housetree.png
;; 
...
;; true if satisfies 1, false if satisfies 2 or "never" if satisfies 3
...
(define (always-before p0 b c)
  ;; acc is Boolean ; whether we have seen b already or not
  (local [(define (fn-for-path p acc)
            (cond [(string=? (path-room p) b)
                   (fn-for-lop (path-nexts p) true)]
                  [(string=? (path-room p) c) acc]
                  [else
                   (fn-for-lop (path-nexts p) acc)]))
          
          (define (fn-for-lop lop acc)
            (cond [(empty? lop) "never"]
                  [else
                   (and-result (fn-for-path (first lop) acc)
                               (fn-for-lop (rest lop) acc))]))]
...
(define (always-before p0 b c)
  ;; todo is (listof (list Path Boolean)) ; worklist accumulator
  ;; result is the combination of everything we've seen so far
  (local [(define (fn-for-path p acc todo result)
            ;; acc is Boolean ; whether we have seen b already or not
            (cond [(string=? (path-room p) b)
                   (fn-for-lop (append (map (λ (e)
                                              (list e true))
                                            (path-nexts p))
                                       todo)
                               result)]
                  [(string=? (path-room p) c)
                   (fn-for-lop todo (and-result acc result))]
                  [else
                   (fn-for-lop (append (map (λ (e)
                                              (list e acc))
                                            (path-nexts p))
