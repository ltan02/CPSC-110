...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname lec20-tree-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require spd/tags)
...
;; Tail recursion with path accumulator in WLEs. Works properly and is TR.
(define (find-path t to)
  ;; todo is (listof WLE); worklist
  ;; visited is (listof String); names of trees to here
  ;; path is (listof String); names of parent, grandparent... in the TREE
  (local [(define-struct wle (t path))
          ;; WLE is (make-wle Tree (listof String))
          ;; interp. a entry in the worklist, saying that fn-for-t must be
          ;;         called on t, with path path
          (define (fn-for-t t todo visited path)
            (local [(define name (node-name t))
              ...
              (if (string=? name to)
                  npath
                  (fn-for-lot (append (map (lambda (s) (make-wle s npath)) subs)
                                      todo)
                              nvisited))))
          ...
          (define (fn-for-lot todo visited)
            (cond [(empty? todo) false]
                  [else
                   (fn-for-t (wle-t (first todo))
                             (rest todo)
                             visited
                             (wle-path (first todo)))]))]
    
    (fn-for-t t empty empty empty)))
