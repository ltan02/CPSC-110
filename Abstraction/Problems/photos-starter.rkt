...
;; produce a list of phoots that are favourites and belong to the given album
(check-expect (to-frame "Victoria" empty) empty)
(check-expect (to-frame "Victoria" (list PHT1 PHT2 PHT3))
              (list PHT1))
(check-expect (to-frame "Seattle" (list PHT1 PHT3 PHT2
                                        (make-photo "" "Seattle" false)))
              (list PHT3))

(define (to-frame s lop)
  (local [(define (correct? p)
            (and (string=? (photo-album p) s)
                 (photo-favourite p)))]
    (filter correct? lop)))
