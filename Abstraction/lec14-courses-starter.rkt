...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname lec14-courses-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;; an abstract fold function for Course
(check-expect (fold-course make-course cons empty C110) C110)

(@template Course (listof Course) encapsulated)
(define (fold-course c1 c2 b c)
...
;(define (all-course-numbers c) empty) ;stub

(define (all-course-numbers c)
  (local [(define (c1 num cr loc) (cons num loc))
          (define (c2 rcourse rloc) (append rcourse rloc))]

    (fold-course c1 c2 empty c)))
...
;(define (total-credits c) 0) ;stub

(define (total-credits c)
  (local [(define (c1 num cr loc) (+ cr loc))
          (define (c2 rcourse rloc) (+ rcourse rloc))]
    (fold-course c1 c2 0 c)))
...
(define (courses-w-credits c n)
  (local [(define (c1 num cr loc)
            (if (>= cr n)
                (cons (make-course num cr loc) loc)
                loc))
          (define (c2 rcourse rloc) (append rcourse rloc))]
    (fold-course c1 c2 empty c)))
