...
;; takes a letter (NOT an entire word) and a Huffman code tree and produces
;; true if that letter is reachable in the given tree, false otherwise. You
;; should not use any accumulators. As a reminder, we have asked you to blend
;; the backtracking template into your design.
...
;; produce the number of odd-length words are in the prefix tree
(check-expect (count-odds PT0) 1)
(check-expect (count-odds (make-pt "T" false empty)) 0)
(check-expect (count-odds PTT) 4)

;(define (count-odds pt) 0) ;stub

(@template PrefixTree (listof PrefixTree) encapsulated accumulator)
(define (count-odds pt)
  ;; acc is Integer ; the amount of letters seen so far
  (local [(define (fn-for-pt pt acc)
            (if (and (pt-f? pt) (odd? acc))
                1
                (fn-for-lopt (pt-subs pt) (add1 acc))))
          
          (define (fn-for-lopt lopt acc)
            (cond [(empty? lopt) 0]
                  [else
                   (+ (fn-for-pt (first lopt) acc)
                      (fn-for-lopt (rest lopt) acc))]))]
    (fn-for-pt pt 1)))
...
;; produce a list of binary digits on the path to a given string, or false
(check-expect (encode "A" "A") (list))
...
(@template HTree encapsulated backtracking accumulator)
(define (encode ht0 s)
  ;; rsf is (listof Integer) ; result-so-far accumulator
  (local [(define (encode ht rsf)
            (cond [(string? ht)
                   (if (string=? ht s)
                       ...
                       false)]
                  [else
                   (local [(define try (encode (htree-zero ht)
                                                    (cons 0 rsf)))]
                     (if (not (false? try))
                         try
                         (encode (htree-one ht) (cons 1 rsf))))]))]
    (encode ht0 empty)))
